...
import { applyOverlay, applyOverlays, type OrgConfig, type Overlay } from "../src/templates/apply.js";

const baseDevShop: OrgConfig = {
  slug: "dev-shop",
  name: "Dev Shop",
  description: "Base",
  architecture: "flat",
...
test("applyOverlay appends members without renaming", () => {
  ...
  assert.deepEqual(
    ...
    ["pm", "fe-eng", "ios-review", "android-review"],
  );
  // Base is untouched.
...
test("applyOverlay appends crons + knowledge_sources", () => {
  ...
  assert.deepEqual(
    merged.crons?.map((c) => c.id).sort(),
    ["backlog-groom", "churn-watch"],
  );
});
...
test("applyOverlays composes in order", () => {
  ...
  assert.deepEqual(
    merged.members.map((m) => m.id),
    ["pm", "fe-eng", "ios-review", "android-review", "cs-agent"],
  );
});
