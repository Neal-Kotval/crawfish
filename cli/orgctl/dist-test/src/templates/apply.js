...
export function applyOverlay(base, overlay) {
    ...
    for (const m of incomingMembers) {
        if (baseMemberIds.has(m.id)) {
            throw new Error(`overlay ${overlay.slug} member id "${m.id}" collides with base ${base.slug}`);
        }
    }
    ...
    for (const c of incomingCrons) {
        if (baseCronIds.has(c.id)) {
            throw new Error(`overlay ${overlay.slug} cron id "${c.id}" collides with base ${base.slug}`);
        }
    }
    ...
    for (const s of incomingSources) {
        if (baseSourceIds.has(s.id)) {
            throw new Error(`overlay ${overlay.slug} knowledge_source "${s.id}" collides with base ${base.slug}`);
        }
    }
    return {
        ...base,
