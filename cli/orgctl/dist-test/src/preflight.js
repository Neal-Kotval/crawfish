...
export const PREFLIGHT_TOOL_DEFS = [
    {
        ...
        description: "Use `preflight_attest` to record that you have read the relevant spec, verified the test fixture, or otherwise completed the preparatory work for a criterion BEFORE you take the action that would satisfy it. Pass `criterion_id` from the task's `criteria` list. Statements must describe what you actually checked, in ≥16 characters. Do NOT preflight without doing the work — the activity log is auditable.",
        inputSchema: {
            type: "object",
            properties: {
                ...
                payload: {
                    type: "object",
                    description: "Optional kind-specific evidence detail (kind, sources, tool_calls, note, etc.).",
                },
            },
...
// ---------- Dispatcher ----------
export async function dispatchPreflight(args, opts = {}) {
