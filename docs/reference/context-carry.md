...
A `Context` is the typed state one agent hands to the next: the accumulated result of a
run so far. A `ContextCarryStrategy` trims that state down before the next agent reads it.
Both live in `crawfish.runtime`. They replace the older approach of stuffing one agent's
output into the next agent's prompt as raw text.

`Context` · `ContextEntry` · `ContextCarryStrategy` · `CarryFull` · `CarryRecency` ·
`CarrySummary` · `CarryTypedFields` · `resolve_carry_strategy`
...
- **`tainted`**: whether this value is *untrusted*. A value is untrusted when it came from
  outside your control (a ticket body, a PR diff, anything fluid).
...
!!! warning "Taint follows the value across agents"

    `tainted` marks untrusted, fluid-derived data, the prompt-injection boundary the
    [security model](../architecture/SECURITY.md) enforces. The framework tracks it so the
    value reaches the next agent as *data to read*, never as *instructions to obey*. A
    carry strategy can drop a tainted entry, but it can never launder one (see
    [Taint survives reduction](#taint-survives-reduction)).

## Choosing how much to carry
...
two results, or only two specific fields. A **`ContextCarryStrategy`** answers that: it takes
a Context and returns a (possibly smaller) one. The four built-ins:
...
!!! warning "Two resolvers, one module"

    `resolve_carry_strategy` shrinks the cross-agent Context artifact; `resolve_strategy`
    shrinks a single agent's transcript. They share a module but take different inputs and
    registries. Reaching for the wrong one silently trims the wrong thing.
...
- `offload_large` / `hydrate`: move oversized values to / from an `ArtifactStore`.
  `hydrate` is the single deref point. Inline by default; an entry offloads only when its
  serialized value exceeds `threshold` (`ARTIFACT_THRESHOLD_BYTES`, 32 768 bytes).
...
- [Core types](core-types.md): `Flow` and the static/fluid boundary taint tracks.
- [Output & wiring](output-and-wiring.md): `Output`, the typed result `add_result` carries into a Context.
- [Persistence](persistence.md): the `Store` and `ArtifactStore` seams `offload_large`, `hydrate`, and `persist` ride on.
