...
Both live in `crawfish.runtime` and replace the old habit of stuffing one agent's output
into the next agent's prompt as raw text.
...
!!! warning "Taint follows the value across agents"
    ...
    [security spine](../architecture/SECURITY.md) enforces. The framework tracks it so the
