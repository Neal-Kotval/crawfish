...
from crawfish.tune import KnobDomain, KnobValue, TuneSpec, tune_spec_sha
...
__all__ = [
    ...
    # Axis-1 types are re-exported from the low-dep crawfish.tune module (same objects).
    "KnobValue",
    "KnobDomain",
    "TuneSpec",
    "tune_spec_sha",
...
# Axis 1 (tunable as data) lives in the low-dep :mod:`crawfish.tune` module so
# ``crawfish.definition.types`` can annotate ``Definition.tune`` without pulling tuner's heavy
# import cycle (tuner → eval → metrics → batch → definition.types). We re-export the SAME class
# objects here (kept in ``__all__``) so ``from crawfish.tuner import TuneSpec`` keeps working and
# ``crawfish.tuner.TuneSpec is crawfish.tune.TuneSpec`` — no isinstance footgun across the seam.
