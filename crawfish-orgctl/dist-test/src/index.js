...
const TOOL_DEFS = [
    {
        ...
        description: "List folded tasks for an org, optionally filtered by status or assignee. Reads the append-only board.jsonl and replays events into Task records.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    },
    {
        ...
        description: "Append a task_created event. Returns the new task_id (ULID). Default status is 'backlog'.",
        inputSchema: {
            type: "object",
            properties: {
                org_id: { type: "string" },
    ...
    },
    {
        ...
        description: "Append a task_updated event. `patch` may include title/description/assignee/status. Idempotent: re-applying the same patch is safe.",
        inputSchema: {
            type: "object",
            properties: {
                org_id: { type: "string" },
    ...
    {
        name: "org_fs_list",
        description: "List entries under the org's hosted files/ directory. Returns { path, kind, size, mtime } sorted by path.",
        inputSchema: {
            type: "object",
            properties: {
                org_id: { type: "string" },
    ...
    {
        name: "org_fs_read",
        description: "Read a single file under <org>/files/. Path-safe; rejects '..', absolute paths, null bytes. 1 MiB max → too_large.",
        inputSchema: {
            type: "object",
            properties: {
                org_id: { type: "string" },
    ...
    {
        name: "org_fs_write",
        description: "Write content to a file under <org>/files/. Creates parent dirs. Idempotent: writing the same content twice is a no-op. 1 MiB max → too_large.",
        inputSchema: {
            type: "object",
            properties: {
                org_id: { type: "string" },
...
async function dispatch(name, args) {
    switch (name) {
        case "board_list_tasks": {
            const tasks = await listTasks(args.org_id, {
                status: args.status,
        ...
        case "board_create_task": {
            const task_id = await createTask(args.org_id, {
                title: args.title,
        ...
        case "org_fs_list": {
            const entries = await listFiles(args.org_id, args.prefix);
            return { entries };
        }
        case "org_fs_read": {
            const r = await readFile(args.org_id, args.path);
            return r;
        }
        case "org_fs_write": {
            const r = await writeFile(args.org_id, args.path, args.content);
            return r;
        }
        default:
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
    ...
    process.stderr.write(`[crawfish-orgctl] ${name} → ${result.tokens_used} tokens${isError ? " (error)" : ""}\n`);
    return {
