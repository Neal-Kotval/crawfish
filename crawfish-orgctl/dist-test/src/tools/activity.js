...
import { commentTask, updateTask, foldTasks, readEvents, } from "../board.js";
...
export const ACTIVITY_TOOL_DEFS = [
    {
        ...
        description: "Post a comment on a task. Appends a task_commented event to board.jsonl and auto-watches the commenter. Returns { tokens_used, ok }.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    },
    {
        ...
        description: "Return the full activity log for a task, including synthesized `mentioned` entries derived from @member patterns in comments. Returns { tokens_used, activity_log }.",
        inputSchema: {
            type: "object",
            properties: {
                org_id: { type: "string" },
    ...
    },
    {
        ...
        description: "Post a comment that explicitly mentions one or more members by id. Appends a task_commented event and auto-watches all mentioned members. Returns { tokens_used, ok }.",
        inputSchema: {
            type: "object",
            properties: {
                org_id: { type: "string" },
...
// ---------- Dispatch ----------
async function activityPostComment(args) {
    const { org_id, task_id, by, body } = args;
...
async function activityListForTask(args) {
    const { org_id, task_id } = args;
    const events = await readEvents(org_id);
    const tasks = foldTasks(events);
    const task = tasks.find((t) => t.id === task_id);
    if (!task) {
    ...
    const enriched = [...task.activity_log];
    for (const ev of events) {
        if (ev.type !== "task_commented" || ev.task_id !== task_id)
            continue;
        const mentions = [];
        let m;
        MENTION_RE.lastIndex = 0;
        while ((m = MENTION_RE.exec(ev.body)) !== null)
            mentions.push(m[1]);
...
async function activityMention(args) {
    const { org_id, task_id, by, mentions, body } = args;
    await commentTask(org_id, { task_id, by, body });
    ...
    if (!task)
        throw new OrgError("not_found", `task ${task_id} not found`);
    const toAdd = [...new Set([by, ...mentions])].filter((m) => !task.watchers.includes(m));
    if (toAdd.length > 0) {
        await updateTask(org_id, {
            task_id,
            by,
...
// ---------- Public dispatcher ----------
export async function dispatchActivity(name, args) {
    try {
        let payload;
        switch (name) {
