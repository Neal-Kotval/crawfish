...
function assertTokens(r: any): void {
...
test("board_create_task round-trips through board_list_tasks", async () => {
  const created: any = await runTool("board_create_task", {
    org_id: ORG_ID,
    title: "First task",
    description: "demo",
    assignee: "founder",
  ...
  const listed: any = await runTool("board_list_tasks", { org_id: ORG_ID });
  assertTokens(listed);
  const found = listed.tasks.find((t: any) => t.id === created.task_id);
...
test("board_update_task is idempotent on retry", async () => {
  ...
  const r1: any = await runTool("board_update_task", {
    org_id: ORG_ID,
  ...
  const r2: any = await runTool("board_update_task", {
    org_id: ORG_ID,
    task_id: created.task_id,
    by: "founder",
    patch,
  ...
  const listed: any = await runTool("board_list_tasks", {
    org_id: ORG_ID,
    status: "in_progress",
  });
  const found = listed.tasks.find((t: any) => t.id === created.task_id);
...
test("board_comment appends and shows up in folded task", async () => {
  ...
  const r: any = await runTool("board_comment", {
    org_id: ORG_ID,
    task_id: created.task_id,
    by: "eng",
    body: "hello",
...
test("org_fs_write + org_fs_read round-trip", async () => {
  ...
  const r: any = await runTool("org_fs_read", {
    org_id: ORG_ID,
    path: "notes/hello.md",
  });
  assertTokens(r);
  ...
  // Idempotent re-write
  const w2: any = await runTool("org_fs_write", {
    org_id: ORG_ID,
    path: "notes/hello.md",
    content: "# Hello\n",
  });
  ...
  const list: any = await runTool("org_fs_list", { org_id: ORG_ID });
  assertTokens(list);
  assert.ok(list.entries.find((e: any) => e.path === "notes/hello.md"));
});
...
test("path escape is rejected", async () => {
  ...
  const r2: any = await runTool("org_fs_write", {
    org_id: ORG_ID,
    path: "/etc/passwd",
    content: "x",
  });
  ...
  const r3: any = await runTool("org_fs_write", {
    org_id: ORG_ID,
    path: "foo\0bar",
    content: "x",
  });
...
test("oversize write rejected as too_large", async () => {
  ...
  const r: any = await runTool("org_fs_write", {
    org_id: ORG_ID,
    path: "big.bin",
    content: big,
  });
...
test("missing org returns not_found", async () => {
  const r: any = await runTool("board_list_tasks", { org_id: "does-not-exist" });
  assert.equal(r.error.code, "not_found");
});

test("invalid status rejected", async () => {
  const r: any = await runTool("board_list_tasks", {
    org_id: ORG_ID,
    status: "weird",
  });
